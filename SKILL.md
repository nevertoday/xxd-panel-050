---
name: xxd-panel-050
description: "Create XXD Panel 050 raster artwork while treating the bundled original style brief as the sole aesthetic authority. Supports one or more top-bottom, left-right, design-only, and four-device wallpaper outputs; multiple ratios or exact sizes; prompt-generated, user-exact, or text-free typography; and inline parameters. Use when the user invokes xxd-panel-050 or requests the exact Panel 050 style."
---

# XXD Panel 050

Create fresh PNG artwork with `references/050-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/050-source.md` completely immediately before building every generation request.
...
```text
/xxd-panel-050 <source> --mode top-bottom,design-only \
...
1. 上下对照（原图在上，050 设计在下）
2. 左右对照（原图在左，050 设计在右）
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/050-source.md,
...
- the result follows `references/050-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
- `references/xxd-panel-050-prompt.zh-CN.md`
- `references/xxd-panel-050-prompt.en.md`

These files document the same minimal adapter. They never replace `references/050-source.md`.
